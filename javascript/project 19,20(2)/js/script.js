var info = [
  ...
  //
  // var DataAll = localStorage.getItem('data');
  ...
  dataService.setData('data', info);
  var DataAll = JSON.parse(dataService.getData('data'));
  ...
  // №1 ARRAY SKILLS
  var arraysAllSkills = _.map(DataAll, "skills");
  var oneArraySkills = _.flatten(arraysAllSkills);
  var uniqSkills =  _.uniq(oneArraySkills);
  var sortSkills =_.sortBy(uniqSkills);


  // console.log(arraysAllSkills);
  // console.log(oneArraySkills);
  // console.log(uniqSkills);
  console.log("1. Список скилов (не повторяющихся): ", sortSkills);
  console.log("------------------------");


  // №2 ARRAY NAME (в зависимости от количества друзей)

  var arraysAllNames = _.map(DataAll, 'name');
  var sortNames = _.sortBy(arraysAllNames, 'friends');

  // console.log("arraysAllNames", arraysAllNames);
  console.log("2. Список имен (в зависимости от количества друзей): ", sortNames);
  console.log("------------------------");


  // №3 ARRAY FRIENDS NAME

  var arraysAllFriends = _.flatMap(DataAll, "friends");
  var nameFriends = _.map(arraysAllFriends, "name");
  var uniqNameFriends = _.uniq(nameFriends);

  // console.log("arraysAllFriendsNames ", arraysAllFriends);
  // console.log("nameFriends ", nameFriends);
  console.log("3. Список имен друзей(не повторяющихся): ", uniqNameFriends);
    ...
    //  var skil = _.sortBy(jsonData, "skills");
  //
    //  for (var i = 0; i < jsonData.length; i++) {
    //    for (var y = 0; y < jsonData[i].skills.length; y++) {
    //      console.log( 'Skills 4el ' + i + ' ' + jsonData[i].skills[y]);
    //     }
    //   }
    //  var skills = _.map(jsonData, 'skills');
